...
if (zqsszbody) {
    ...
    let zq_ssz_path = resolve(__dirname, zq_ssz_file);
    zqsszbody = fs.readFileSync(zq_ssz_path, "utf8");
    if (zqsszbody !== `undefined`) {
        zqsszbodys = zqsszbody.split("\n");
    } else {
        $.msg($.name, '【提示】请点击搜索赚某一任务获取body', '不知道说啥好', {"open-url": "给您劈个叉吧"});
        $.done()
    }
}
                ...
                if(result.success === true ){
                    ...
                    if(comstate === 1){
                        ...
                        for(let i = 0;i<4;i++){
                            ...
                            await $.wait(sleep_time);
                            await look()
                            sleep_time = Math.floor(Math.random() * (1500 - 1000 + 1000) + 10000);
                            console.log(`\n随机等待 ${sleep_time/1000} 秒\n`)
                            await $.wait(sleep_time);
                            await look()
                            sleep_time = Math.floor(Math.random() * (1500 - 1000 + 1000) + 10000);
