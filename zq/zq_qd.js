...
if (zqqdbody) {
    ...
    let zq_qd_path = resolve(__dirname, zq_qd_file);
    zqqdbody = fs.readFileSync(zq_qd_path, "utf8");
    if (zqqdbody !== `undefined`) {
        zqqdbodys = zqqdbody.split("\n");
    } else {
        $.msg($.name, '【提示】请签到以获取body，明天再跑一次脚本测试', '不知道说啥好', {"open-url": "给您劈个叉吧"});
        $.done()
    }
}
    ...
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/CommonReward\/toGetReward/)) {
        ...
        if (zqqdbody) {
            if (zqqdbody.indexOf(bodyVal) > -1) {
                ...
                $.msg($.name, ``, `获取第 ${bodys.length} 个签到&任务宝箱奖励: 成功🎉`)
            }
        } else {
            $.setdata(bodyVal, 'zqqdbody');
            $.log(`${$.name}获取签到: 成功, zqqdbodys: ${bodyVal}`);
            $.msg($.name, ``, `获取第 1 个签到&任务宝箱奖励: 成功🎉`)
        }
    }
