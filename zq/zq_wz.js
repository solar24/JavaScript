...
let zq_timebodyArr = []
let zq_timebodys = "", zqwzbody1, zq_timebody1, msg, allScore
...
if (zq_timebody) {
    ...
    let zq_wz_time_path = resolve(__dirname, zq_wz_time_file);
    zq_timebody = fs.readFileSync(zq_wz_time_path, "utf8");
    if (zq_timebody !== `undefined`) {
        zq_timebodys = zq_timebody.split("\n");
    } else {
        $.msg($.name, '【提示】请点击文章阅读1分钟获取timebody，再跑一次脚本', '不知道说啥好', {"open-url": "给您劈个叉吧"});
        $.done()
    }
}
...
if (zqwzbody) {
    ...
    const fs = require("fs");
    const { resolve } = require('path');
    let zq_wz_path = resolve(__dirname, zq_wz_file);
    zqwzbody = fs.readFileSync(zq_wz_path, "utf8");
    if (zqwzbody !== `undefined`) {
        zqwzbodys = zqwzbody.split("\n");
    } else {
        $.msg($.name, '【提示】请点击文章获取body，再跑一次脚本', '不知道说啥好', {"open-url": "给您劈个叉吧"});
        $.done()
    }
}
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        for (let k =  0; k < zqwzbodyArr.length; k++) {
            ...
            await wzjl(k + 1)
            let sleep_time = Math.floor(Math.random() * (15000 - 10000 + 1000) + 60000);
...
function getzqwzbody() {
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/article\/info.json/)||$request.url.match(/\/kandian.wkandian.com\/v5\/article\/detail.json/)) {
        ...
        if (zqwzbody) {
            ...
            $.log(`${$.name}获取阅读: 成功, zqwzbodys: ${bodyVal}`);
            $.msg($.name, ``, `获取第 1 个阅读请求: 成功🎉`)
        }
    }

}
//阅读文章奖励
function wzjl(number, timeout = 0) {
    ...
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/user\/stay.json/)) {
        ...
        if (zq_timebody) {
            if (zq_timebody.indexOf(bodyVal) > -1) {
                ...
                $.msg($.name, ``, `获取第 ${bodys.length} 个阅读时间请求: 成功🎉`)
            }
        } else {
            $.setdata($request.body,'zq_timebody');
            $.log(`${$.name}获取阅读: 成功, zq_timebodys: ${bodyVal}`);
            $.msg($.name, ``, `获取第 1 个阅读时间请求: 成功🎉`)
        }
    }
}
