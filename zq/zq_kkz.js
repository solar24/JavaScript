...
let zq_cookieArr = []
let zq_cookies = "", cookie, cookie_id, zq_cookie1, msg, zqlookStartbody1, allScore, successNum, failNum, doNum;
...
if (zqlookStartbody) {
    ...
    const fs = require("fs");
    const { resolve } = require('path');
    let zq_kkz_path = resolve(__dirname, zq_kkz_file);
    zqlookStartbody = fs.readFileSync(zq_kkz_path, "utf8");
    if (zqlookStartbody !== `undefined`) {
        zqlookStartbodys = zqlookStartbody.split("\n");
    } else {
        $.msg($.name, '【提示】请点击看看赚某一任务获取body', '不知道说啥好', {"open-url": "给您劈个叉吧"});
        $.done()
    }
}
...
async function getZqLookStartbody() {
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/nameless\/adlickstart/)) {
        ...
        if (zqlookStartbody) {
            ...
            $.log(`${$.name}获取看看赚任务: 成功, zqlookStartbodys: ${bodyVal}`);
            $.msg($.name, ``, `获取第 1 个看看赚任务请求: 成功🎉`)
        }
    }

}
//看看赚激活
function lookStart(number, timeout = 0) {
