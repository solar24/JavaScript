...
15 22 * * *  https://raw.githubusercontent.com/solar24/JavaScript/main/zq/zq_today_score.js 中青收益统计
...
let bodyVal, cookie, cookie_id, zq_cookie1, zq_cookie2, nickname1;
...
!(async () => {
    ...
    for (let k = 0; k < zq_cookieArr.length; k++) {
        ...
        console.log(`\n--------第 ${k + 1} 个账号风险评测中--------\n`)
        await do_getOrderList();
        sleep_time = Math.floor(Math.random() * (1500 - 1000 + 1000) + 4000);
        console.log(`\n随机等待 ${sleep_time/1000} 秒\n`)
        await $.wait(sleep_time);
        console.log("\n\n")
    }
...
function do_getOrderList() {
    return new Promise((resolve) => {
        ...
        const url = {
            ...
            headers: {
                'Host': 'kd.youth.cn',
                'Referer': 'https://kd.youth.cn/html/withdraw/order.html?' + cookie_id
            },
            body: cookie_id
        }
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if (result.status === '1') {
                    ...
                    // 最新数据
                    let latest = txArr[0];
                    let add_time_str = latest.add_time_str;
                    let description = latest.description === '' ? '账号暂无风险' : latest.description;
                    let money = latest.money;
                    let status ;
