...
34 9 * * * https://raw.githubusercontent.com/solar24/JavaScript/main/zq/zq_llz.js 中青浏览赚
...
if (zqllzbody) {
    ...
    let zq_llz_path = resolve(__dirname, zq_llz_file);
    zqllzbody = fs.readFileSync(zq_llz_path, "utf8");
    if (zqllzbody !== `undefined`) {
        zqllzbodys = zqllzbody.split("\n");
    } else {
        $.msg($.name, '【提示】请点击浏览赚某一任务获取body', '不知道说啥好', {"open-url": "给您劈个叉吧"});
        $.done()
    }
}
;
...
async function getzqllzbody() {
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/task\/browse_start/)) {
        ...
        if (zqllzbody) {
            ...
            $.log(`${$.name}获取浏览赚任务: 成功, zqllzbodys: ${bodyVal}`);
            $.msg($.name, ``, `获取第 1 个浏览赚任务请求: 成功🎉`)
        }
    }

}
//浏览赚激活
function Start(number, timeout = 0) {
