/*
脚本源地址：https://raw.githubusercontent.com/shaolin-kongfu/js_scripts/main/zq/zqbox.js

3 * * * * https://raw.githubusercontent.com/solar24/JavaScript/main/zq/zq_box.js 中青定时宝箱领取
...
if (zqboxbody) {
    ...
    let zq_box_path = resolve(__dirname, zq_box_file);
    zqboxbody = fs.readFileSync(zq_box_path, "utf8");
    if (zqboxbody !== `undefined`) {
        zqboxbodys = zqboxbody.split("\n");
    } else {
        $.msg($.name, '【提示】请在app首页，点击右上角图标获取一次数据', '不知道说啥好', {"open-url": "给您劈个叉吧"});
        $.done()
    }
}
    ...
    if ($request.url.match(/\/kandian.wkandian.com\/v5\/CommonReward\/toGetReward/)) {
        ...
        if (zqboxbody) {
            if (zqboxbody.indexOf(bodyVal) > -1) {
                ...
                $.msg($.name, ``, `获取第 ${bodys.length} 个定时宝箱: 成功🎉`)
            }
        } else {
            $.setdata(bodyVal, 'zqboxbody');
            $.log(`${$.name}获取宝箱: 成功, zqboxbodys: ${bodyVal}`);
            $.msg($.name, ``, `获取第 1 个定时宝箱: 成功🎉`)
        }
    }
        ...
        $.post(url, async (err, resp, data) => {
            ...
            try {
                ...
                if (result.success !== false) {
                    ...
                    console.log(`\n领取宝箱奖励失败: ${JSON.stringify(result)}`)
                }
            } catch (e) {
                console.log(data);
                $.logErr(e, resp);
