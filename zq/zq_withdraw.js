/*
脚本源地址：https://raw.githubusercontent.com/shaolin-kongfu/js_scripts/main/zq/zq_withdraw.js

25 10 * * * https://raw.githubusercontent.com/solar24/JavaScript/main/zq/zq_withdraw.js 中青自动提现

***************************************************************************
v2p食用说明：
1.添加重写如下：
https://kandian.wkandian.com/v5/wechat/withdraw2.json 重写目标 https://raw.githubusercontent.com/shaolin-kongfu/js_scripts/main/zq/zq_withdraw.js
...
let zq_cookieArr = [], zq_cookies = "", now_money, time1;
let bodyVal, cookie, cookie_id, zq_cookie1, zq_cookie2, zq_withdraw1, nickname1;
...
if (zq_withdraw) {
    ...
    let zq_withdraw_path = resolve(__dirname, zq_withdraw_file);
    zq_withdraw = fs.readFileSync(zq_withdraw_path, "utf8");
    if (zq_withdraw !== `undefined`) {
        zq_withdraws = zq_withdraw.split("\n");
    } else {
        $.msg($.name, '【提示】请先完成一次提现，明天再跑一次脚本', '不知道说啥好', {"open-url": "给您劈个叉吧"});
        $.done()
    }
}
